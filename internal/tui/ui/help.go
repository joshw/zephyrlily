...
var tuiHelp = map[string][]string{
	...
	"shorten": {
		...
		"",
		"Usage: %shorten [s.u13.net|da.gd|tinyurl]",
		"",
		...
		"",
		"Services:",
		"  s.u13.net  the default: tigerlily's cj.pl shortener, so a link made",
		"             here is the link the bot would have made. It needs a key,",
		"             which released builds carry; without one it answers 403.",
		"             Set ZLILY_SHORTEN_API_KEY to use your own key instead.",
		"  da.gd      no account needed; the one to switch to if s.u13.net balks",
	...
	"lastcol": {
		"Keep the input area off the terminal's last column",
		"",
		"Usage: %debug lastcol [on|off]",
		"",
		"Two display bugs in mosh 1.4.0 are both reached the same way: by",
		"writing to the very last column of the bottom row. Doing that leaves",
		"the terminal in the 'pending wrap' state, which mosh then loses track",
		"of - once by discarding it on a mode change, once by counting a",
		"backspace from a column the cursor cannot really be in. Either way the",
		"client's idea of the cursor drifts one cell from the server's, and",
		"lines start overwriting each other.",
		"",
		"The bottom row is where the input line lives, so it is the only row",
		"that can carry the state into the next frame - every other row gets",
		"re-anchored. Holding the input one column short of the edge means we",
		"never create the state, so neither bug can fire.",
		"",
		"It is OFF by default, because the empty column is visible and most",
		"connections are not mosh. If a mosh-server is running on the machine",
		"when you log in, zlily says so once and leaves the choice to you - after",
		"the login prompts are answered and the output has settled, so it does",
		"not land in the middle of the blurb prompt or the review.",
		"",
		"That check cannot tell whether YOUR session is the mosh one. Nothing",
		"can: mosh leaves no marker in the environment, screen hides it from the",
		"process tree, and asking the terminal to identify itself does not settle",
		"it either - mosh answers that it is a plain VT220, and so do other",
		"terminals. So the note is always phrased as a maybe. Treat it as a",
		"prompt to decide, not as a diagnosis.",
		"",
		"This prevents the bugs, where '%debug redraw' only cleans up after",
		"them. Both are only needed on an unpatched mosh; the fixes are in",
		"display-bug-repro/upstream.",
		"",
		"The snapshot records the setting.",
	},
	"redraw": {
		"Force a full repaint when the input area shrinks",
		"",
		"Usage: %debug redraw [on|off]",
		"",
		"When the input area shrinks back from two lines to one, the viewport",
		"grows to reclaim the row. A display bug has been seen on that",
		"transition, and forcing a full repaint there makes it go away.",
		"",
		"It is ON by default. Testing across six terminal/transport/screen",
		"combinations found the corruption in every path carrying mosh and in",
		"none without it, whichever terminal was used and whether or not screen",
		"was in the way - so the repaint is a workaround for mosh, and the",
		"cost of it is one extra repaint when the input line gets shorter.",
		"",
		"Turn it OFF to observe the bug - a full repaint clears any display",
		"divergence whatever the cause, so leaving it on hides the failure. If",
		"you are filing a bug report, say which way it was set: a clean",
		"snapshot taken with it on proves nothing. The snapshot records the",
		"setting either way.",
		"",
		"This cleans up after the mosh bugs; '%debug lastcol' prevents them.",
		"",
		"If the display does go wrong, run '%debug snapshot' BEFORE resizing",
		"or pressing C-l - both repaint, and the repaint destroys the",
		"evidence.",
	},
	"page": {
	...
	"password": {
		"Save your Lily password so you do not have to type it",
		"",
		"Usage: %save-password              save this session's password",
		"       %forget-password [user]     remove a saved password",
		"",
		"The login dialog has a 'Remember password' box; ticking it does the",
		"same thing as %save-password, and clearing it on a later login removes",
		"what was stored. Your username is remembered either way — that is why",
		"the dialog opens with the cursor already on the password.",
		"",
		"None of this applies in a browser: there is no home directory and no",
		"keyring to reach, so the box is not shown and these commands report",
		"that they have nowhere to write. A reload does not ask you to log in",
		"again anyway — the page re-attaches to the session it already had.",
		"",
		"There are two places a password can be kept, and zlily says which one",
		"it used:",
		"",
		"  - your OS keyring (macOS Keychain, Windows Credential Manager, or a",
		"    freedesktop Secret Service), preferred when the machine has one;",
		"  - ~/.config/zlily/credentials, mode 0600, when it does not — which is",
		"    the usual case on a headless host reached over ssh.",
		"",
		"The file is read first, so a line you add there by hand overrides the",
		"keyring. It is tab-separated, one account per line, and you can edit it",
		"in any editor. zlily ignores it entirely if anyone else can read it;",
		"chmod 600 it and the ignoring stops.",
		"",
		"The password in that file is plain text, because Lily authenticates in",
		"plain text on every connect: there is nothing to hash and no token to",
		"keep instead. It is protected by the file mode, not by encryption, so",
