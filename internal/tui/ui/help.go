...
var tuiHelp = map[string][]string{
	...
	"lastcol": {
		...
		"connections are not mosh. zlily looks for mosh at login and suggests",
		"turning it on if it finds any - once the login prompts are answered and",
		"the output has settled, so the suggestion does not land in the middle of",
		"the blurb prompt or the review.",
		"",
		"It asks the terminal to identify itself, which mosh answers with a",
		"signature of its own - so if nothing sits between zlily and mosh, the",
		"answer is certain. Under screen or tmux the multiplexer answers that",
		"query instead and mosh is invisible, so zlily falls back to looking for",
		"a mosh-server process on the machine. That one cannot tell whether YOUR",
		"session is the mosh one, and it says so: treat a hedged suggestion as a",
