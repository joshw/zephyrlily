...
var tuiHelp = map[string][]string{
	...
	"redraw": {
		...
		"",
		"It is OFF by default, because making it go away is not the same as",
		"fixing it: a full repaint clears any display divergence whatever the",
		"cause, so leaving this on hides the very failure we are trying to",
		"catch. It was originally added believing the cause was a bug in mosh;",
		"that no longer holds - the corruption has since appeared with no mosh",
		"in the path at all.",
		"",
		"Turn it on if a session becomes unusable. If you do, say so on any",
		"bug report: a snapshot taken with it on cannot show the problem. The",
		"snapshot records the setting either way.",
		"",
